using CodingChallenges;
using System;
using System.Collections.Generic;
using System.Text;

namespace CodingChallenges.AdventOfCode.Year2025
{
    public class Challenge05 : Challenge
    {
        public override void Solve()
        {
            List<long[]> ranges = [];
            List<long> ingredients = [];

            long countFresh = 0;

            foreach(string line in Input)
            {
                if (line.Length > 0)
                {
                    if (line.Contains('-'))
                    {
                        string[] r = line.Split(['-'], StringSplitOptions.RemoveEmptyEntries);
                        ranges.Add([long.Parse(r[0]), long.Parse(r[1])]);
                    }
                    else
                    {
                        ingredients.Add(long.Parse(line));
                    }
                }
            }

            foreach(long id in ingredients)
            {
                foreach (long[] range in ranges)
                {
                    if ((id >= range[0]) && (id <= range[1]))
                    {
                        countFresh++;
                        break;
                    }
                }
            }

            Part1Solution = countFresh.ToString();

            List<long[]> sortedRanges = [];

            foreach (long[] range in ranges)
            {
                int i;
                for (i = 0; i < sortedRanges.Count; i++)
                {
                    if (range[0] < sortedRanges[i][0])
                    {
                        sortedRanges.Insert(i, range);
                        break;
                    }
                }
                if (i == sortedRanges.Count)
                {
                    sortedRanges.Add(range);
                }
            }

            for (int i = 0; i < sortedRanges.Count - 1; i++)
            {
                if (sortedRanges[i + 1][0] <= sortedRanges[i][1])
                {
                    if (sortedRanges[i + 1][1] <= sortedRanges[i][1])
                    {
                        sortedRanges.RemoveAt(i-- + 1);
                    }
                    else
                    {
                        sortedRanges[i][1] = sortedRanges[i + 1][1];
                        sortedRanges.RemoveAt(i-- + 1);
                    }
                }
            }

            long idCount = 0;
            foreach (long[] range in sortedRanges)
            {
                idCount += range[1] - range[0] + 1;
            }

            Part2Solution = idCount.ToString();

            Part3Solution = "";
        }
    }
}
